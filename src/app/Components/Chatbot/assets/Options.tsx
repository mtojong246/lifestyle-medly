...
import { addChat } from "@/app/api/firebase";
...
export default function Options({...props}) {
    ...
    const optionsMarkup = options.map((option) => (
        <button key={option.id} onClick={option.handler} className='p-[10px] text-sm rounded bg-none border m-1 hover:bg-maize hover:border-maize hover:text-white'>
            {option.text}
        </button>
    ));
    ...
    useEffect(() => {
        const saveChat = async () => {
            if (props.state.email && props.state.name) {
                try {
                    await addChat({
        ...
        saveChat();
    }, [props.state.email, props.state.name])
